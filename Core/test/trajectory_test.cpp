...
bool keyPointsTest() { //true is fail
    ...
    try {
        if(abs(v1.at(0)-0)>DEADBAND) return true;
        if(abs(v1.at(1)-0.238929)>DEADBAND) return true;
        if(abs(v1.at(2)-0.91106)>DEADBAND) return true;
        if(abs(v1.at(3)-0.99009)>DEADBAND) return true;
        if(abs(v1.at(4)-1)>DEADBAND) return true;
    } catch(int e) {
        return true;
    }
    return false;
